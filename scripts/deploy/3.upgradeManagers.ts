...
export const main = async () => {
  ...
  try {
    ...
    for (const chain of chains) {
      ...
      const tm = (await socketContract.transmitManager__()).toLowerCase();
      if (tm !== chainAddresses[CORE_CONTRACTS.TransmitManager].toLowerCase()) {
        const tx = await socketContract.setTransmitManager(
          chainAddresses[CORE_CONTRACTS.TransmitManager]
        );
        console.log("setting tm for", chain, tx.hash);
        await tx.wait();
      }
