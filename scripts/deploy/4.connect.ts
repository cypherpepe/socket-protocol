...
import WatcherABI from "../../out/Watcher.sol/Watcher.json";
  ...
  try {
    ...
    await Promise.all(
      chains.map(async (chain) => {
        ...
        const watcherVMaddr: ChainSocketAddresses = addresses[3605]!;
...
export const updateConfigWatcherVM = async () => {
  let addresses: DeploymentAddresses;
  try {
    console.log("Connecting plugs on Watcher");
    addresses = dev_addresses as unknown as DeploymentAddresses;
    ...
    const watcher = new Contract(
      watcherVMaddr[WatcherVMCoreContracts.Watcher],
      WatcherABI.abi,
      signer
    );
    await Promise.all(
    ...
    console.log(`Updating Watcher Config tx hash: ${tx.hash}`);
    await tx.wait();
  } catch (error) {
    console.log("Error while sending transaction", error);
  }
